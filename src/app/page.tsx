...
export default function HomePage() {
  ...
  const { data: randomStories, isLoading: randomStoriesLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const { data, error } = await supabase.rpc('get_random_stories', {
          limit_count: 3,
        });
  ...
  const { data: topGenres, isLoading: topGenresLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const { data, error } = await supabase.from('histoires_genres').select('genre:genre_id(nom), count:genre_id', {
          count: 'exact',
        });
        if (error) throw error;
        const newData = data
  ...
  const { data: lastStories, isLoading: lastStoriesLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const { data, error } = await supabase.from('histoires').select('*').order('created_at', { ascending: false }).limit(4);
        if (error) throw error;
      ...
      <main>
              ...
              randomStories.map((slide, idx) => (
                <SwiperSlide key={idx}>
                  <div className="relative w-full h-full">
                    <img src={slide.image} alt={slide.titre} className="absolute top-0 left-0 w-full h-full object-cover" />
                    <div className="relative z-10 flex items-center h-full bg-black/50">
                      <div className="max-w-xl text-white p-6 md:p-10">
                        <h2 className="text-2xl md:text-3xl font-bold mb-4">{slide.titre}</h2>
                        <p className="text-gray-100">
                          {slide.contenu.substring(0, 100)}
                          ...
                        </p>
                        <Link
        ...
        <section className="flex flex-col gap-4 text-center items-center px-6 py-20">
          <header>
            <h1 className="text-5xl max-sm:text-2xl font-bold text-white">
              ...
              <span className="bg-gradient-to-r from-violet-400 to-violet-600 text-transparent bg-clip-text">une histoire,</span> une claque
              !
            </h1>
          </header>
        ...
        <section className="mx-60 max-[1400px]:mx-10 mb-20">
          ...
          <div className="grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-md:grid-cols-1">
            {topGenresLoading && Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} height="100%" />)}
            {topGenres &&
              topGenres.map((genre, index) => (
                <article key={index} className="w-full">
                  <Link href={`/histoires?genre=${genre.nom}`}>
                    <HistoireCard
                      histoireWithImage={{
                        id: '',
                        image: genresMap.find((g) => g.nom === genre.nom)?.img || '',
                        titre: genre.nom,
                        contenu: '',
                        auteur: '',
                        created_at: '',
        ...
        <section className="mx-60 max-[1400px]:mx-10 mb-20">
          ...
          <div className="flex flex-row gap-4 w-full max-lg:flex-col">
            <article className="w-2/3 max-lg:w-full">
              ...
              {lastStories && (
                <Link href={`/histoires/${lastStories[0].slug}`}>
                  <HistoireCard histoireWithImage={lastStories[0]} showContent={true} props="h-full" imageProps="h-2/3 object-cover" />
                </Link>
              )}
            </article>
            <aside className="w-1/3 flex flex-col gap-4 max-lg:w-full max-lg:grid max-lg:grid-cols-2 max-md:grid-cols-1">
              {lastStoriesLoading && Array.from({ length: 3 }).map((_, index) => <Skeleton key={index} height="100%" />)}
              {lastStories?.map((histoireWithImage, index) => {
                if (index === 0) return null;
                return (
                  <article key={index} className="w-full">
                    <Link href={`/histoires/${histoireWithImage.slug}`}>
                      <HistoireCard histoireWithImage={histoireWithImage} />
                    </Link>
                  </article>
                );
              })}
