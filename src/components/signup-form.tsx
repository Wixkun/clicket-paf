    ...
    return (
      <main className={`max-w-md mx-auto p-6 ${className}`} {...props}>
        <section className="flex flex-col items-center justify-center p-8 text-center">
          <h1 className="text-2xl font-bold mb-4 text-white">Vérifiez vos emails !</h1>
          <p className="text-gray-300 mb-2">Un email de confirmation a été envoyé à votre adresse.</p>
          <p className="text-gray-300">Cliquez sur le lien dans l'email pour activer votre compte.</p>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className={`max-w-md mx-auto p-6 ${className}`} {...props}>
      <header className="text-center mb-6">
        ...
        <p className="text-gray-300">Entrez vos informations pour créer un compte.</p>
      </header>
      <section>
      ...
      <footer className="mt-4 text-center text-sm">
        <p className="text-white">
          ...
          <Link href={`/login?redirectTo=${encodeURIComponent(redirectTo)}`} className="underline underline-offset-4 text-violet-400">
            connectez-vous
          </Link>
        </p>
      </footer>
